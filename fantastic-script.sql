...
JOIN `task3`.`authors` ON `authors`.`id` = `authors_books`.`id_author`) JOIN `task3`.`genres_book` ON `books`.`id` = `genres_book`.`id_book`)                                                   
JOIN `task3`.`genre` ON `genres_book`.`id_genre` = `genre`.`id`
