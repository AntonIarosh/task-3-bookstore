...
JOIN `task3`.`authors` ON `authors`.`id` = `authors_books`.`id_author`) JOIN `task3`.`genre` ON `books`.`id_genre` = `genre`.`id` 
GROUP BY `authors`.`id`;
...
JOIN `task3`.`authors` ON `authors`.`id` = `authors_books`.`id_author`) JOIN `task3`.`genre` ON `books`.`id_genre` = `genre`.`id` GROUP BY `authors`.`id`)y);